...
;;
;; Sometimes the WSH will complain that it doesn't have an engine for js files
;; if this is the case, you need to give js files the proper file association
;; inside of a windows command shell, type assoc .js
;; That will tell you what is currently associated.  Then use
;; assoc .js=JSFile
...
(define-derived-mode inferior-wsh-mode comint-mode "Inf-wsh"
  ...
  :syntax-table (cond ((boundp 'js2-mode-syntax-table) js2-mode-syntax-table)
					  ((boundp 'espresso-mode-syntax-table) espresso-mode-syntax-table)
					  ((boundp js-mode-syntax-table) js-mode-syntax-table)
					  (t (warn "Warning! Cannot find suitable syntax table for inferior wsh mode")))
